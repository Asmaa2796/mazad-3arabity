...
const BrandAuctions = () => {
    ...
    useEffect(() => {
        dispatch(fetchAuctions({ type: 'all', page, brand_id: brandId }));
    }, [dispatch, page, brandId]);

    useEffect(() => {
        setPage(1);
    }, [dataLanguage, language, brandId]);
    ...
    return (
        <div className={`${style.auctions} py-5`}>
            <div className="container">
                <h2 className="text-center fw-medium mb-4">{t.auctions?.all || 'All Auctions'}</h2>
                {auctionsState.status === "loading" ? (
                    ...
                    <div className="text-center py-5 alert alert-warning">
                        <h5>{t.auctions?.noAuctions || 'No auctions found for this brand'}</h5>
                    </div>
                ) : (<>
                    <div className="row">
