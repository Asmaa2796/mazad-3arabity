...
const MyBids = () => {
    ...
    if (auctionsState.status === "loading") {
        return <div className="text-center py-5">{t.common.loading}</div>;
    }

    if (auctionsState.error) {
        return <div className="alert alert-danger my-3">{auctionsState.error}</div>;
    }

    return (
        <div className={`${style.auctions} py-5`}>
            <div className="container">
                <h2 className="text-center fw-medium mb-4">{t.profile.myBids}</h2>
                <div className="row">
