...
import { acceptOffer, fetchAuctionDetails, postBid } from "../../../Redux/Slices/auctionsSlice";
...
const AuctionDetails = () => {
  ...
  // bids
  useEffect(() => {
    const unsubscribe = listenAuctionBids(id, async (bids) => {
      const bidsWithUsers = await Promise.all(
        bids.map(async (bid) => {
          if (!bid.user_id) return bid;

          const userRef = doc(db, "users", String(bid.user_id));
        ...
        <div className={`${style.auction_details} py-5`}>
          <div className="container">
            ...
            <div className={`${style.car_tabs}`}>
              <div className="my-5">
                <h5>{record?.brand?.name} - {record?.model}</h5>
